...
class SatNet{
    ...
    // ***************************************************
    // *************************************************** 
    // BST Operation operations 
    int find_sat_balance(Sat *satellite ); // finds the balance of any two sats
    int find_sat_height(Sat *aDrone);  //calculates the height of the passed satellite in the tree
    int preOrder_sat_count(INCLIN degree, Sat *satCounter) const;
    bool preOrder_findSatellite(int id, Sat *traverseSat) const;
    bool inOrdersetState(int id, STATE state, Sat *stateSat);
    Sat* satelliteBalance(Sat *balanceSat);//returns the balance of any sat thats passed into it
    Sat* insertPreorder(const Sat& insertion_sat , Sat *mainSat); // recurssively inserts nodes in order into the tree
    void adjust_sat_height(Sat *satellite);
    void postOrderclear(Sat *destructorSat); // post order destruction/deletion
    void inOrder_listSatellites(Sat *traversalSat) const;
    void inOrderremoveDeorbited(Sat *orbitSat);
    Sat* inOrderRemove(int id, Sat *removalSat);
    Sat* findMin(Sat* minSat); //finds the leftmost node in the right subtree of the node we want to remove. otherwise known as the inorder successor
    Sat* postOrdercopySat(Sat *assignSat); // aids in helping to create a deep copy of the tree and its nodes
    void removeState(STATE state);
    void inOrderremoveState(Sat *orbitSat, STATE state);
    
    // Msin AVL operations 
    Sat* rotateLeft(Sat *root);// function that handles left rotations of satellites in the tree
    Sat* rotateRight(Sat *root);//function that handles right rotations of satellites in the tree
